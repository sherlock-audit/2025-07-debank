...
contract MachaV2Adapter is ReentrancyGuard {
    address public immutable MachaV2_Approver;
    address public immutable MachaV2_Router;
    ...
    constructor(address approver, address router) {
        MachaV2_Approver = approver;
        MachaV2_Router = router;
    }
    ...
    function swapOnAdapter(
        ...
        if (fromToken != UniversalERC20.ETH) {
            // approve
            IERC20(fromToken).forceApprove(MachaV2_Approver, fromTokenAmount);
            MachaV2_Router.functionCallWithValue(data, 0);
        } else {
            // For ETH, use fromTokenAmount instead of msg.value to handle fee deduction
            MachaV2_Router.functionCallWithValue(data, fromTokenAmount);
        }
