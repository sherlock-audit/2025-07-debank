...
contract ParaswapAdapter is ReentrancyGuard {
    address public immutable Paraswap_Approver;
    address public immutable Paraswap_Router;
    ...
    constructor(address approver, address router) {
        Paraswap_Approver = approver;
        Paraswap_Router = router;
    }
    ...
    function swapOnAdapter(
        ...
        if (fromToken != UniversalERC20.ETH) {
            // approve
            IERC20(fromToken).forceApprove(Paraswap_Approver, fromTokenAmount);
            Paraswap_Router.functionCallWithValue(data, 0);
        } else {
            // For ETH, use fromTokenAmount instead of msg.value to handle fee deduction
            Paraswap_Router.functionCallWithValue(data, fromTokenAmount);
        }
